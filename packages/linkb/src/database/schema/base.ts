...
export abstract class BaseSchemaGenerator {
  ...
  public async generateSchema(): Promise<void> {
    ...
    try {
      console.log(chalk.blue('Generating schema from collections configuration...'));
...
`;
      ...
      for (const [tableName, tableSchema] of Object.entries(schema)) {
        ...
        const hasReferences = Object.values(tableSchema).some(col => 
   ...
   */
  private getDrizzleColumnType(columnDef: ColumnDefinition, columnName: string): string {
    ...
    if (isSnakeCase) {
      ...
      switch (type) {
        ...
        default:
          ...
          console.warn(chalk.yellow(`Unknown column type: ${type}, defaulting to text()`));
          columnDeclaration = `t.text("${columnName}")`;
      }
    } else {

...
export const execute = async (action: string, args?: string[]): Promise<void> => {
  ...
  if (!loadEnv(workspaceRoot)) {
    ...
    console.log(chalk.yellow("Create a .env file with DATABASE_TYPE and connection details"));
    return;
  }
  ...
  if (!databaseType) {
    ...
    console.log(chalk.yellow("Add DATABASE_TYPE to your .env file. Supported values:"));
  ...
  if (!SUPPORTED_DATABASES.includes(databaseType.toLowerCase() as SupportedDatabase)) {
  ...
  try {
    ...
    const dbConfig = {
      ...
      migrationDir: process.env.MIGRATION_DIR || `${workspaceRoot}/apps/web/database/migrations`,
    ...
    if (!dbConfig.connectionString) {
      ...
      console.log(chalk.yellow("Please add DATABASE_URL to your .env file with a valid connection string"));
      console.log(chalk.yellow("Example: postgresql://username:password@localhost:5432/database"));
      return;
    }
    ...
    if (!connectionStringLower.startsWith('postgresql://') && !connectionStringLower.startsWith('postgres://')) {
      console.log(chalk.yellow("Warning: Your DATABASE_URL doesn't start with 'postgresql://' or 'postgres://'"));
      console.log(chalk.yellow("This might cause connection issues. Check your connection string format."));
    }
    
    // Log configuration (without sensitive data)
    console.log(chalk.blue('Database configuration:'));
    console.log(chalk.blue(`  - Migration directory: ${dbConfig.migrationDir}`));
    console.log(chalk.blue(`  - Table: ${dbConfig.tableName}`));
    if (dbConfig.schema) console.log(chalk.blue(`  - Schema: ${dbConfig.schema}`));
    ...
    switch (action) {
      ...
      case "status":
        ...
        status.forEach(migration => {
          const statusColor = migration.status === 'applied' ? chalk.green : chalk.yellow;
          console.log(`${statusColor(migration.status)} - ${migration.name}${migration.batch ? ` (batch ${migration.batch})` : ''}`);
        });
        await adapter.close();
        break;
      ...
      default:
        ...
        console.log(chalk.blue("  - rollback: Rollback the last batch of migrations"));

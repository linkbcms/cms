...
const createSettingsValidation = z.object({
  ...
  select: z.enum(['option1', 'option2', 'option3']).optional().nullable(),
});
...
async function getSettings(id: string) {
  ...
  if (result.length === 0) {
    return NextResponse.json({ name: 'settings not found' }, { status: 404 });
  }
...
async function patchSettings(id: string, req: NextRequest) {
  ...
  try {
    ...
    const result = await db
      .update(settings)
      .set(payload)
      .where(eq(settings.id, Number.parseInt(id, 10)))
      .returning();
    if (result.length === 0) {
